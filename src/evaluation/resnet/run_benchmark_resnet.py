...
from experiments.resnet.train_resnet_base import (
    update_config as update_config_resnet18_pxDiff,
)
from experiments.resnet.train_resnet_conditional_norm_all_px import (
    update_config as update_config_resnetCN18_pxNone,
)
from evaluation.utils.evaluators.resnet_evaluator import ResnetEvaluator
...
def prepare_training_dataloader(
    ...
    fold = 0
    if mode == "resnet18_px":
        data, checkpoint_path, config = update_config_resnet18_pxDiff(
            Path(DATA_DIR),
            Path(OUTPUTS_MODELS),
            fold,
            checkpoint_path,
    ...
    if mode == "resnetCN18":
        data, checkpoint_path, config = update_config_resnetCN18_pxNone(
            Path(DATA_DIR),
            Path(OUTPUTS_MODELS),
            fold,
            checkpoint_path,
...
if __name__ == "__main__":
    ...
    if args.model_type == "resnetCN18":
        data_module, config = prepare_training_dataloader(
            "resnetCN18", 1, args.model_checkpoint_path, None, args.unique_name
        )
    else:
        data_module, config = prepare_training_dataloader(
            "resnet18_px",
    ...
    num_conditions = (
        config["model"]["num_conditions"]
        if args.model_type in ["resnetCN18"]
        else None
    )
    ...
    with torch.no_grad():
        ...
        for _ in range(BENCHMARK_MEASURE_BATCHES + num_warmup):
            ...
            try:
                ...
                if evaluator.model_type == "resnet18":
                    _ = evaluator.model(x_dummy)
                elif evaluator.model_type in ["resnetCN18"]:
                    _ = evaluator.model(x_dummy, pixel_size_dummy)
                else:
                    raise ValueError(
                        f"UNKNOWN MODEL TYPE {evaluator.model_type} IN RESNET BENCHMARK"
                    )
