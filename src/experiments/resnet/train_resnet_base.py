...
def update_config(
    ...
    update_trainer_config(
        config,
        output_models,
        f"resnet18_fold_{args_fold}_pixel_size_{pixel_size_str}_{arg_unique_name}",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Dir for saving outputs, logs from torch lighting. Pass unique text to add to dirname: resnet18_fold_{args.fold}_pixel_size_{pixel_size_str}_{GIVEN_DIR_NAME}",
    )
    parser.add_argument(
        "--remove_BG",
        type=int,
