...
def update_config(
    ...
    update_trainer_config(
        config, output_models, f"unetCN_fold_{args_fold}_{arg_unique_name}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Dir for saving outputs, logs from torch lighting. Pass unique text to add to dirname: unetCN_fold_{args.fold}_{GIVEN_DIR_NAME}",
    )
    parser.add_argument(
        "--min_pixel_size",
        type=float,
